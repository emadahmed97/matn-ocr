"""
DeepSeek OCR Data Collator for Arabic text processing.

Extracted from notebooks/arabic_ocr_finetune.ipynb cell #22.
Handles image-text pair processing for OCR training.
"""
...
# Import DeepSeek-OCR specific utilities
try:
    ...
    print("Warning: DeepSeek-OCR utilities not found. Please ensure deepseek_ocr is available.")
...
class DeepSeekOCRDataCollator:
    ...
    Processes image-text pairs for Arabic OCR training with dynamic preprocessing
    and proper attention masking.

    Args:
        tokenizer: Tokenizer for text processing
        model: Model for extracting dtype and device info
...
def convert_to_conversation(sample, instruction: str = "<image>\nFree OCR. "):
    """
    Convert dataset sample to conversation format required by DeepSeek-OCR.

    Args:
        sample: Dataset sample with 'image' and 'text' fields
        instruction: Instruction prompt for OCR task
